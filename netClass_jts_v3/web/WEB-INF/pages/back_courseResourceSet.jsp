    ...
    <script type="text/javascript" >
        $(function(){
            ...
            $(".comment-detail").on('click', function(){
                ...
                $.ajax({
                    url:'${pageContext.request.contextPath}/comment/back/findByParams.do',
                    data:{'resourceId':resourceId},
                    dataType:'json',
                    type:'post',
                    success:function(result){
                        if(result.success){
                            $('#frame-id', window.parent.document).attr('src', '${pageContext.request.contextPath}/showBackComment.do');
                        }
                        else{
                            alert('服务有问题');
                        }
    ...
    <div class="panel-body">
        ...
        <div class="show-list">
            <table class="table table-bordered table-hover" style='text-align: center;'>
                ...
                <tbody id="tb">
                <c:forEach items="${pageChapters}" var="chapter">
                    <tr>
                        <td>${chapter.id}</td>
                        <td>${chapter.title}</td>
                        <td>${chapter.info}</td>
                        ...
                        <c:if test="${chapter.status eq 0}">
                            ...
                            <td class="text-center">
                                ...
                                <input resourceId="${chapter.resource.id}" type="button" class="btn btn-success btn-sm resource-detail" value="详情" />
                                <input resourceId="${chapter.resource.id}" type="button" class="btn btn-success btn-sm comment-detail" value="相关评论" />
                            </td>
                        </c:if>
                ...
                <!--假数据：点击相关评论，将该章节的资源id（1）去数据库中查询此章节的所有评论 -->
                <tr>
                    <td>1</td>
                    <td>java基础</td>
                    <td>java基础相关</td>
                    ...
                    <td class="text-center">
                        ...
                        <input resourceId="1" type="button" class="btn btn-success btn-sm comment-detail" value="相关评论" />
                    </td>
                </tr>
                </tbody>
            </table>
