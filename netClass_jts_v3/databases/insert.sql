...
CREATE TABLE `t_user` (
  ...
  `login_date` datetime DEFAULT NULL COMMENT '最近一次签到的日期',
...
insert into t_course_type (type_name,status) values
('英语',0);
insert into t_course_type (type_name,parent_id,status) values
('初中英语',1,0);
insert into t_course_type (type_name,parent_id,status) values
('初中英语作文',2,0);
insert into t_course_type (type_name,parent_id,status) values
('初中英语阅读',2,0);
insert into t_course (course_name,course_info,author,create_date,click_number,status,
course_type_id) values
('英语满分作文','aaa','J.K.Rolin',now(),0,0,3);
insert into t_course (course_name,course_info,author,create_date,click_number,status,
course_type_id) values
('议论文','aaa','J.K.Rolin',now(),1,0,3);
insert into t_course (course_name,course_info,author,create_date,click_number,status,
course_type_id) values
('记叙文','aaa','J.K.Rolin',now(),2,0,3);
insert into t_user (login_name,nickname,password,role,email,login_date,create_date,status)
values
('js','js','js','normal','aaa',null,now(),0);
insert into t_chapter (course_id,title,info,create_date,status) values
(1,'111','111',now(),0);
insert into t_resource (title,path,original_name,file_size,click_count,create_date,
cost_type,cost_number,user_id,chapter_id,status)values
('aaa','aaa','aaa',1024,0,now(),0,1,1,1,0);
insert into t_user_resource (user_id,resource_id,create_date,update_date) values
(1,1,now(),now());
insert into t_comment(context,create_date,user_id,resource_id,status) values
('aaa',now(),1,1,0);
insert into t_comment(context,create_date,user_id,resource_id,status) values
('bbb',now(),1,1,0);
insert into t_comment(context,create_date,user_id,resource_id,status) values
...
insert into t_comment(context,create_date,user_id,resource_id,status) values
('fff',now(),1,1,2);
insert into t_praise (user_id,comment_id,create_date)values
(1,1,now());